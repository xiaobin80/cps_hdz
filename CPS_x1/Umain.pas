...
type
  ...
  private
    ...
    iniFilePath:string;
    count1,count2,count3:integer;//计数器
...
var
  ...
  //
  TXTfilename:string;//传递给Utxt.pas
  tp:TSearchRec;
  handlers:string;
...
begin
  ...
  begin
    TempStr := '';
    for iFor := 0 to colCount-1 do//列数
    begin
      if iFor=0 then
      begin
...
begin
 ...
 Except
   Application.MessageBox('数据库已改变，请数据库管理员重新配置！','提示',MB_OK+MB_ICONINFORMATION);
   Exit;
 end;
   //
...
begin
   //
   StatusBar1.Panels[1].Text:='铁路值班员';
   StatusBar1.Panels[2].Text:='登录时间：'+RightStr(DateTimeToStr(now),8);
   //建立文件夹(更改过的报文)
  ...
  case sw_int of
    ...
    2:
      begin
        ...
        printcounter1:=IntToStr(QuickRep1.PageNumber);
        StatusBar1.Panels[0].Text:='正在打印，请稍候... ...';
        StatusBar1.Panels[1].Text:='打印页数：'+printcounter1+'页';
    
        //QuickRep1.Print;   //现场用
        QuickRep1.Print;//2006.9.12

        //显示打印相关信息2
        StatusBar1.Panels[0].Text:='等待下一次打印！';

        ShellExecute(frm_main.Handle,'open','delfile1.bat',nil,excpchar,SW_SHOWNORMAL);
        //执行sp-UpdateLMD_h12point
...
begin
  ...
  try
    ...
    Application.MessageBox('出现严重系统问题2，请检查报文:"13754037902.txt"！','系统退出提示',MB_OK);
...
begin
  ...
  try
    ...
    Application.MessageBox('出现严重系统问题4，请检查报文:"13754037904.txt"！','系统退出提示',MB_OK);
    Application.Terminate;
  end;
...
begin
  ...
  try
    ...
    Application.MessageBox('出现严重系统问题6，请检查报文:"13754037907.txt"！','系统退出提示',MB_OK);
    Application.Terminate;
  end;
end;
...
begin
 if Application.MessageBox('是否退出系统？','提示',MB_YESNO+MB_ICONQUESTION)=IDYES then
 begin
   Application.Terminate;
 end;
end;
